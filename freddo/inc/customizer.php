...
function freddo_custom_settings_register( $wp_customize ) {
	...
	*/
	...
	$wp_customize->add_setting('freddo_theme_options[_googlefonts_locally]', array(
        'default'    => '',
        'type'       => 'option',
        'capability' => 'edit_theme_options',
		'sanitize_callback' => 'freddo_sanitize_checkbox'
    ) );
	$wp_customize->add_control('freddo_theme_options[_googlefonts_locally]', array(
        'label'      => __( 'Host Google Fonts locally', 'freddo' ),
        'section'    => 'cresta_freddo_theme_options_general',
        'settings'   => 'freddo_theme_options[_googlefonts_locally]',
        'type'       => 'checkbox',
		'priority' => 3,
    ) );
	...
	*/
	...
	for( $number = 1; $number < FREDDO_VALUE_FOR_SERVICES; $number++ ){
		...
		$wp_customize->add_setting('freddo_theme_options[_onepage_optbutton_'.$number.'_services]', array(
			'default'    => '',
			'type'       => 'option',
			'capability' => 'edit_theme_options',
			'sanitize_callback' => 'sanitize_text_field'
		) );
		$wp_customize->add_control('freddo_theme_options[_onepage_optbutton_'.$number.'_services]', array(
			'label'      => __( 'Button text (optional)', 'freddo' ),
			'section'    => 'cresta_freddo_onepage_section_services',
			'settings'   => 'freddo_theme_options[_onepage_optbutton_'.$number.'_services]',
			'active_callback' => 'freddo_is_services_active',
			'type'       => 'text',
		) );
		/* Optional link in service button */
		$wp_customize->add_setting('freddo_theme_options[_onepage_optbutton_link_'.$number.'_services]', array(
			'default'    => '',
			'type'       => 'option',
			'capability' => 'edit_theme_options',
			'sanitize_callback' => 'esc_url_raw'
		) );
		$wp_customize->add_control('freddo_theme_options[_onepage_optbutton_link_'.$number.'_services]', array(
			'label'      => __( 'Button link (optional)', 'freddo' ),
			'section'    => 'cresta_freddo_onepage_section_services',
			'settings'   => 'freddo_theme_options[_onepage_optbutton_link_'.$number.'_services]',
			'type'       => 'url',
			'active_callback' => 'freddo_is_services_active',
		) );
	}
