...
<?php if ($showServices == 1) : ?>
	<?php
		...
		for( $number = 1; $number < FREDDO_VALUE_FOR_SERVICES; $number++ ){
			...
			$singleServiceButton["$number"] = freddo_options('_onepage_optbutton_'.$number.'_services', '');
			$singleServiceButtonLink["$number"] = freddo_options('_onepage_optbutton_link_'.$number.'_services', '');
		}
	?>
					...
					<?php if ($singleServiceBox["$number"]) : ?>
						<div class="singleService">
							...
							<div class="serviceText">
								...
								<?php endif; ?>
								<?php if($singleServiceButton["$number"]): ?>
									<div class="serviceButton freddoButton">
										<a href="<?php echo esc_url($singleServiceButtonLink["$number"]); ?>"><?php echo esc_html($singleServiceButton["$number"]); ?></a>
									</div>
								<?php endif; ?>
							</div>
						</div>
